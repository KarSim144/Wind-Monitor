...
// Repeater 1 validates each record, drops duplicates/old records, and forwards
...
const uint8_t PREVIOUS_MAC[6] = {0x68, 0x09, 0x47, 0x9D, 0x08, 0xE0};
const uint8_t OWN_MAC[6] = {0x20, 0x9B, 0xA9, 0x68, 0xAB, 0x9C};
const uint8_t NEXT_MAC[6] = {0x68, 0x09, 0x47, 0x9D, 0x32, 0xE4};
}  // namespace Config
...
// Adds the next hop as an unencrypted ESP-NOW peer.
...
// Accepts only Sensor DATA; newer valid IDs move forward immediately.
...
void printStatsWhenDue() {
  ...
  Serial.printf("#R1 forwarded=%lu duplicate=%lu rejected=%lu "
...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  WiFi.disconnect();
  if (!ownMacIsCorrect()) stopWithError("FATAL: wrong Repeater 1 MAC.");
  ...
  if (esp_wifi_set_ps(WIFI_PS_NONE) != ESP_OK ||
    ...
    stopWithError("FATAL: Repeater 1 setup failed.");
  }

  nextStatsAt = millis() + 1000;
  Serial.println("#REPEATER 1 REALTIME READY");
}
